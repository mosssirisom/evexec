...
async function sendNotification(req, res) {
  ...
  const emailHtml = emailLayout({ title: isReminder ? 'Upcoming Transfer — EV Exec' : 'Transfer Confirmed — EV Exec', body: `<p style="margin:0 0 6px;font-family:Inter,Arial,sans-serif;font-size:15px;color:#fff">Hi ${firstName},</p><p style="margin:0 0 20px;font-family:Inter,Arial,sans-serif;font-size:15px;color:rgba(255,255,255,.65);line-height:1.6">${isReminder ? 'A reminder about your upcoming EV Exec airport transfer.' : 'Your airport transfer is confirmed.'}</p><h2 style="margin:0 0 4px;font-family:Inter,Arial,sans-serif;font-size:19px;font-weight:700;color:#fff;line-height:1.3">${route}</h2><p style="margin:0 0 16px;font-family:Inter,Arial,sans-serif;font-size:14px;color:rgba(255,255,255,.6)">${date} at ${time} &nbsp;&middot;&nbsp; ${booking.passengers || 1} passenger(s)</p><p style="margin:0;font-family:Inter,Arial,sans-serif;font-size:14px;color:rgba(255,255,255,.65)">Payment: <strong style="color:#fff">${method}</strong></p>` });
