...
function fmtDate(dateStr) {
  ...
  try {
    // dateStr is a plain YYYY-MM-DD calendar date with no time component, so
    // `new Date(dateStr)` parses it as UTC midnight. Formatting without an
    // explicit timeZone falls back to whatever zone the server process is
    // running in -- on a host west of UTC that silently rolls the display
    // back to the previous calendar day (a customer sees "24 September" for
    // a booking stored as the 25th). Always format in UTC so the date shown
    // is exactly the date stored, regardless of server locale.
...
function journeyLine(b) {
  ...
  const stopLines = stops.map((s, i) => `  Stop ${i + 1}: ${s}`);
  if (!b.return_journey) {
    return [outbound, ...stopLines].join('\n');
  }
...
function buildOperatorJobOfferSms(booking, acceptUrl, rejectUrl) {
  ...
  if (booking.return_journey && booking.return_date) {
    ...
    legs.forEach((leg, i) => {
      ...
      if (leg.label === 'OUTBOUND' && stops.length) {
        stops.forEach((s, si) => lines.push(`  Stop ${si + 1}: ${s}`));
      }
...
module.exports = { fmtDate, fmtTime, journeyLine, journeySummaryText, lookupPrice, getPrice, PRICES, shortenAddress, buildOperatorJobOfferSms, emailJourneyHtml, refBadgeHtml, getStops, singleLineSubject };
