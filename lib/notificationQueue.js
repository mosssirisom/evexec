...
async function processItem(item) {
  if (item.channel === 'sms') {
    await sendSMS(item.recipient, item.body || 'EV Exec notification');
  } else if (item.channel === 'email') {
    await sendEmail({ to: item.recipient, subject: item.subject || 'EV Exec notification', html: item.html || item.body || '' });
  } else {
