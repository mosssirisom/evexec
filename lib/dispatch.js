...
// Returns the date (YYYY-MM-DD) `delta` days away from `dateStr`
...
async function tryAutoDispatch(booking) {
  ...
  const [drivers, prevBookings, dayBookings, nextBookings] = await Promise.all([
    getOnlineDrivers(),
    getBookingsOnDate(addDays(travel_date, -1)),
    getBookingsOnDate(travel_date),
    getBookingsOnDate(addDays(travel_date, 1)),
  ]);

  // Normalise each day's bookings onto a shared minute-of-day axis (today = 0)
  // so jobs that cross midnight are still detected as conflicts.
  const allBookings = [
    ...prevBookings.map(b => ({ b, offset: -1440 })),
    ...dayBookings.map(b => ({ b, offset: 0 })),
    ...nextBookings.map(b => ({ b, offset: 1440 })),
  ];

  const available = drivers.filter(driver =>
    !allBookings.some(({ b, offset }) =>
      b.assigned_driver_id === driver.id &&
      b.id !== id &&
      overlaps(startMins, estimatedDuration, (toMins(b.travel_time) ?? -9999) + offset, 90)
    )
  );
