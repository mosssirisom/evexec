...
// VAPID_PRIVATE_KEY is the standard raw 32-byte EC scalar produced by
// `npx web-push generate-vapid-keys` (per .env.example) — NOT a PKCS8 DER
// blob. Node has no "raw EC private key" import mode, so rebuild it as a
// JWK using the scalar plus the public key's x/y coordinates.
function vapidPrivateKeyFromRaw(privateKeyB64, publicKeyB64) {
  ...
  return crypto.createPrivateKey({
    key: { kty: 'EC', crv: 'P-256', d: privateKeyB64, x, y },
    format: 'jwk'
  });
}
...
async function makeVapidJwt(audience, subject, publicKeyB64, privateKeyB64, expSeconds = 43200) {
  ...
  const key = vapidPrivateKeyFromRaw(privateKeyB64, publicKeyB64);
...
async function sendPushToOperator(title, message, actionUrl) {
  ...
  if (!vapidPublicKey || !vapidPrivateKey || !operatorUserId) return;

  const res = await fetch(
    `${SUPABASE_URL()}/rest/v1/push_subscriptions?user_id=eq.${operatorUserId}&select=*`,
    { headers: dbHeaders() }
  );
  if (!res.ok) return;
...
module.exports = { sendWebPush, getSubscriptions, deleteExpiredSubscription, sendPushToCustomer, sendPushToOperator };
