...
async function sendSMS(to, body) {
  const customerId = process.env.SMSWORKS_CUSTOMER_ID;
  const apiKey     = process.env.SMSWORKS_API_KEY;
  const sender     = process.env.SMS_SENDER || 'EVExec';
  if (!customerId || !apiKey) { console.warn('SMS Works not configured — SMS skipped'); return; }
  ...
  try {
    const authRes = await fetch('https://api.thesmsworks.co.uk/v1/auth/token', {
      ...
      body: JSON.stringify({ customerid: customerId, secret: apiKey })
    });
    const authText = await authRes.text();
    if (!authRes.ok) { console.error('SMS Works auth error', authRes.status, authText); return; }
    authData = JSON.parse(authText);
    if (!authData.token) { console.error('SMS Works auth: no token in response', authText); return; }
  } catch (e) { console.error('SMS Works auth exception:', e.message); return; }
  ...
  try {
    const smsRes = await fetch('https://api.thesmsworks.co.uk/v1/message/send', {
      ...
      body: JSON.stringify({ message: { sender, destination: to, content: body } })
    });
    const smsText = await smsRes.text();
    if (!smsRes.ok) { console.error('SMS Works send error', smsRes.status, smsText); return; }
    console.log('SMS sent to', to, JSON.parse(smsText).status || '');
  } catch (e) { console.error('SMS Works send exception:', e.message); }
}
