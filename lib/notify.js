...
// Handles both the website's lowercase values ('card'/'cash') and the
// operator dashboard's fuller Title-Case vocabulary (see evexecoperator
// migration 009_expand_payment_method_values.sql).
function paymentMethodLabel(paymentMethod) {
  const labels = {
    ...
    Cash: 'Cash on the day',
    Card: 'Paid by card',
    'Card machine': 'Card (paid on arrival)',
    'Payment link': 'Paid online',
    'Bank transfer': 'Paid by bank transfer',
  };
  return labels[paymentMethod] || (paymentMethod ? paymentMethod : 'TBC');
}
...
async function sendConfirmations(booking, notes = '') {
  ...
  const method = paymentMethodLabel(booking.payment_method);
...
<div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto">
  ...
  const logEntries = [];
  if (booking.customer_phone) logEntries.push(['sms', normaliseUkPhone(booking.customer_phone)]);
...
// Sent to the CUSTOMER once a Stripe payment link is actually paid — a
// standard "payment successful" confirmation + receipt, separate from
// sendConfirmations() (which confirms the booking itself, sent earlier in
// the flow). SMS + a receipt-style branded email. Stripe's own official
// receipt (if enabled in the Stripe Dashboard under Customer emails) is a
// separate, complementary email — this one matches the rest of EV Exec's
// branded correspondence rather than relying solely on Stripe's generic one.
async function sendPaymentReceived(booking) {
  const route = journeyLine(booking);
  const date = fmtDate(booking.travel_date);
  const price = getPrice(booking);
  const firstName = (booking.customer_name || 'there').split(' ')[0];
  const paidAt = fmtDate(new Date().toISOString());
  const amountTxt = price != null ? `£${price}` : 'your balance';

  const customerSms =
    `EV Exec: Hi ${firstName}, we've received your payment of ${amountTxt} for your ` +
    `airport transfer (Ref ${booking.ref}). Thank you! See you on ${date}. ` +
    `Questions: 07721 070370`;

  const customerEmailHtml = `
<div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto">
  <div style="background:#10b981;padding:20px 28px;border-radius:12px 12px 0 0">
    <h1 style="margin:0;color:#06101c;font-size:1.2rem">Payment Successful — EV Exec</h1>
  </div>
  <div style="background:#020813;color:#fff;padding:28px;border-radius:0 0 12px 12px">
    <p style="margin:0 0 6px">Hi ${firstName},</p>
    <p style="margin:0 0 20px;color:rgba(255,255,255,.65)">Thank you — we've received your payment. Your booking is fully paid and confirmed.</p>
    <div style="border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:16px 18px;margin:0 0 20px">
      <p style="margin:0 0 4px;color:rgba(255,255,255,.5);font-size:12px;text-transform:uppercase;letter-spacing:.04em">Receipt</p>
      <p style="margin:0 0 2px"><strong style="color:#fff">Reference:</strong> <span style="color:rgba(255,255,255,.75)">${booking.ref}</span></p>
      <p style="margin:0 0 2px"><strong style="color:#fff">Journey:</strong> <span style="color:rgba(255,255,255,.75)">${route}</span></p>
      <p style="margin:0 0 2px"><strong style="color:#fff">Travel date:</strong> <span style="color:rgba(255,255,255,.75)">${date} at ${booking.travel_time || 'TBC'}</span></p>
      <p style="margin:0 0 2px"><strong style="color:#fff">Amount paid:</strong> <span style="color:#10b981;font-weight:700">${amountTxt}</span></p>
      <p style="margin:0"><strong style="color:#fff">Paid on:</strong> <span style="color:rgba(255,255,255,.75)">${paidAt}</span></p>
    </div>
    <p style="color:rgba(255,255,255,.5);font-size:13px">Questions? Call or WhatsApp: <a href="tel:07721070370" style="color:#d5a538">07721 070370</a></p>
  </div>
</div>`;
  ...
  if (booking.customer_phone) {
    tasks.push(sendSMS(booking.customer_phone, customerSms));
    logEntries.push(['sms', normaliseUkPhone(booking.customer_phone)]);
  }
  if (booking.customer_email) {
    tasks.push(sendEmail({
      to: booking.customer_email,
      subject: `Payment Received — Receipt for Ref ${booking.ref}`,
      html: customerEmailHtml
    }));
  ...
  if (settled.some(r => r.status === 'fulfilled')) {
    await logMany(booking.id, 'payment_received', logEntries).catch(() => {});
  }
  return settled;
}

module.exports = { sendSMS, sendEmail, sendConfirmations, sendRejectionNotice, sendPaymentReceived, normaliseUkPhone, paymentMethodLabel };
