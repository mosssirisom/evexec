...
const { journeyLine, fmtDate, fmtTime, getPrice, emailJourneyHtml, refBadgeHtml } = require('./format');
...
async function sendConfirmations(booking, notes = '', receiptUrl = null) {
  ...
  await Promise.allSettled([
    ...
    hasCustomerEmail
      ...
      : (hasCustomerPhone ? sendOrQueue(() => sendSMS(booking.customer_phone, customerSms), { booking_id: booking.id, type: 'confirmation', channel: 'sms', recipient: booking.customer_phone, body: customerSms }) : null),
    ...
    opEmail ? sendOrQueue(() => sendEmail({ to: opEmail, subject: `Payment Confirmed: ${route}, ${date}`, html: operatorEmailHtml }), { booking_id: booking.id, type: 'confirmation', channel: 'email', recipient: opEmail, subject: `Payment Confirmed: ${route}, ${date}`, html: operatorEmailHtml }) : null,
    !opEmail && process.env.OPERATOR_PHONE ? sendOrQueue(() => sendSMS(process.env.OPERATOR_PHONE, operatorSms), { booking_id: booking.id, type: 'confirmation', channel: 'sms', recipient: process.env.OPERATOR_PHONE, body: operatorSms }) : null,
...
async function sendRejectionNotice(booking) {
  ...
  await Promise.allSettled([
    hasEmail
      ...
      : (hasPhone ? sendOrQueue(() => sendSMS(booking.customer_phone, customerSms), { booking_id: booking.id, type: 'rejection', channel: 'sms', recipient: booking.customer_phone, body: customerSms }) : null),
